...
import { ManualMatchAnalysis, UserRole, AnalysisData } from '../types';

interface HubPageProps {
  manualHistory: ManualMatchAnalysis[];
  aiHistory: AnalysisData[]; // Novo prop para análises de IA
  userRole: UserRole;
  onAddNew: () => void;
  onSelectAnalysis: (analysis: ManualMatchAnalysis | AnalysisData) => void;
  onDelete: (item: ManualMatchAnalysis | AnalysisData) => void;
}

const HubPage: React.FC<HubPageProps> = ({ manualHistory, aiHistory, userRole, onAddNew, onSelectAnalysis, onDelete }) => {
  const canWrite = userRole === 'admin' || userRole === 'analista';
  const canDelete = userRole === 'admin';

  // Combinar e ordenar todas as análises por data de criação (ou ID para simplificar)
  const combinedHistory = [...manualHistory, ...aiHistory].sort((a, b) => {
    // Assumir que IDs gerados têm alguma ordem ou usar um campo de data real se disponível
    // Para simplificar, vou usar o ID como string para ordenação básica
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {canWrite && (
          ...
          >
            ...
            <span className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.2em] group-hover:text-zinc-400 transition-colors">Criar Novo Registo Manual</span>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className="flex flex-col gap-1">
                {/* Diferenciar tipo de análise */}
                {'videoSource' in item ? (
                  <span className="text-[9px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded uppercase w-fit">Manual: {item.date}</span>
                ) : (
                  <span className="text-[9px] font-black text-orange-500 bg-orange-500/10 px-2 py-1 rounded uppercase w-fit">IA Gerado</span>
                )}
                <span className="text-[8px] font-bold text-zinc-600 uppercase mono tracking-tighter">
                  {'location' in item ? item.location : `Gerado em ${new Date().toLocaleDateString()}`}
                </span>
              </div>
              {canDelete && (
                <button 
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1 bg-zinc-950 rounded-full overflow-hidden border border-white/5">
                    {/* Exibir progresso ou tipo de análise */}
                    {'videoSource' in item ? (
                       <div className="h-full bg-emerald-600" style={{ width: `${Math.min(item.events.length, 100)}%` }}></div>
                    ) : (
                       <div className="h-full bg-orange-600" style={{ width: `100%` }}></div>
                    )}
                  </div>
                  <span className="text-[8px] font-black text-zinc-500 uppercase mono">
                    {'videoSource' in item ? `${item.events.length} EVTS` : 'AI REPORT'}
                  </span>
                </div>
              </div>
