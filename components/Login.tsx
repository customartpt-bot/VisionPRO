  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (data.user) {
         ...
         let roleToSave = 'dashboard';

         // Lógica de Salvaguarda: Se o username indicar a role, usa-a (útil se o perfil na BD não estiver sincronizado)
         if (username.toLowerCase().includes('analistaind')) {
             roleToSave = 'analistaind';
         } else if (username.toLowerCase().includes('analistacol')) {
             roleToSave = 'analistacol';
         } else if (username.toLowerCase().includes('admin')) {
             roleToSave = 'admin';
         } else if (profile) {
             roleToSave = profile.role;
         }
      ...
      <div className="bg-dark-surface p-10 rounded-xl shadow-2xl w-full max-w-sm border border-dark-border relative z-10">
        ...
        <div className="mt-8 pt-6 border-t border-dark-border">
            <div className="flex justify-between text-[10px] text-gray-600 font-mono">
                <span>SYSTEM: CUSTOMART PRO SERIES</span>
