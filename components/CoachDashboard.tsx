...
const PlayerStatsRow: React.FC<{ player: Player, events: MatchEvent[], currentMatchMinute: number }> = ({ player, events, currentMatchMinute }) => {
    ...
    if (startMinute !== null) minutesPlayed = Math.max(0, endMinute - startMinute);
...
const CoachDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (matchData) {
          ...
          setIsTimerRunning(matchData.is_timer_running || false);
      }
  ...
  const stats = useMemo(() => {
    ...
    return {
      home: { 
          ...
          red: calc('home', 'red_card'), loss: calc('home', 'ball_loss'), recovery: calc('home', 'ball_recovery'), corners: calc('home', 'corner'),
      },
      away: { 
          ...
          red: calc('away', 'red_card'), loss: calc('away', 'ball_loss'), recovery: calc('away', 'ball_recovery'), corners: calc('away', 'corner'),
      }
    };
  }, [events]);
  ...
  const getTeamSquad = (team: 'home' | 'away') => {
      ...
      teamPlayers.forEach(p => {
          ...
          const isOnPitch = p.is_starter ? !subOut : (!!subIn && !subOut);
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(ev => {
      if (!ev.player_id) return true;
      const allowedIndividualTypes: EventType[] = ['goal', 'assist', 'sub_in', 'sub_out', 'yellow_card', 'red_card'];
      return allowedIndividualTypes.includes(ev.type);
    });
  }, [events]);
    ...
    <div className="flex flex-col h-screen bg-dark-bg text-gray-200 overflow-hidden font-sans">
      <div className="bg-black border-b border-dark-border h-16 flex items-center justify-between px-6 shrink-0 z-20 relative">
        ...
        <div className="absolute left-1/2 top-0 -translate-x-1/2 bg-dark-surface h-full flex flex-row items-center justify-center gap-4 px-6 border-x border-dark-border/50 shadow-2xl min-w-[500px]">
           <div className="text-sm font-bold text-gray-400 uppercase tracking-wide text-right w-32 truncate hidden md:block">{match?.home_team || 'HOME'}</div>
           ...
           <div className="text-sm font-bold text-gray-400 uppercase tracking-wide text-left w-32 truncate hidden md:block">{match?.away_team || 'AWAY'}</div>
           ...
           <div className="flex items-center text-xl font-mono font-bold text-gray-200 leading-none bg-black px-3 py-1 rounded border border-dark-border">
              ...
              <span className={`mx-0.5 text-brand ${isTimerRunning ? 'animate-pulse' : 'opacity-50'}`}>:</span>
        ...
        <div className="w-1/3 flex justify-end items-center gap-4">
           ...
           <div className="text-right text-[10px] text-gray-600 font-mono uppercase truncate w-32">{match?.competition}</div>
        </div>
      </div>
              ...
              {activeTab === 'collective' ? (
                 <div className="flex flex-col gap-6">
                     <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 content-start">
                          <StatCard title="Posse Real" icon={Clock} homeValue={formatTimeSimple(match?.possession_home || 0)} awayValue={formatTimeSimple(match?.possession_away || 0)} helperText="Tempo" />
                         ...
                         <div className="flex-1 overflow-y-auto p-0 divide-y divide-dark-border">
                             {filteredEvents.map(ev => (
                                <div key={ev.id} onClick={() => playerRef.current?.seekTo(ev.video_timestamp, 'seconds')} className="p-3 flex items-center justify-between hover:bg-dark-surface cursor-pointer group transition-colors">
                                   ...
                                   <button className="text-gray-500 hover:text-brand transition"><PlayCircle size={14} /></button>
                                </div>
                              ))}
                         </div>
                     </div>
                  ...
                  <div className="grid grid-cols-2 gap-8 h-full overflow-hidden">
                     <div className="flex flex-col h-full overflow-hidden">
                        <div className="bg-dark-surface p-2 border-b border-dark-border mb-2 sticky top-0"><h4 className="text-white font-bold text-xs uppercase tracking-widest border-l-4 border-brand pl-2">{match?.home_team}</h4></div>
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                             <div className="mb-6">
                                <h5 className={`text-[10px] font-bold uppercase mb-2 flex items-center gap-2 justify-between ${homeSquad.active.length > 11 ? 'text-red-500 animate-pulse' : 'text-green-500'}`}>
                                    <div className="flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${homeSquad.active.length > 11 ? 'bg-red-500' : 'bg-green-500 animate-pulse'}`}></div>Em Jogo ({homeSquad.active.length})</div>
                                </h5>
                                <div className="space-y-1">
                                    {homeSquad.active.map(player => ( <PlayerStatsRow key={player.id} player={player} events={events} currentMatchMinute={currentMatchMinute} /> ))}
                                </div>
                             </div>
                             <div className="opacity-80">
                                <h5 className="text-[10px] text-gray-500 font-bold uppercase mb-2 border-t border-dark-border pt-2">Banco</h5>
                                <div className="space-y-1">
                                    {homeSquad.bench.map(player => ( <PlayerStatsRow key={player.id} player={player} events={events} currentMatchMinute={currentMatchMinute} /> ))}
                                </div>
                             </div>
                        </div>
                     </div>
                     <div className="flex flex-col h-full overflow-hidden">
                        <div className="bg-dark-surface p-2 border-b border-dark-border mb-2 sticky top-0"><h4 className="text-gray-400 font-bold text-xs uppercase tracking-widest border-l-4 border-gray-600 pl-2">{match?.away_team}</h4></div>
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                             <div className="mb-6">
                                <h5 className={`text-[10px] font-bold uppercase mb-2 flex items-center gap-2 justify-between ${awaySquad.active.length > 11 ? 'text-red-500 animate-pulse' : 'text-green-500'}`}>
                                    <div className="flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${awaySquad.active.length > 11 ? 'bg-red-500' : 'bg-green-500 animate-pulse'}`}></div>Em Jogo ({awaySquad.active.length})</div>
                                </h5>
                                <div className="space-y-1">
                                    {awaySquad.active.map(player => ( <PlayerStatsRow key={player.id} player={player} events={events} currentMatchMinute={currentMatchMinute} /> ))}
                                </div>
                             </div>
                             <div className="opacity-80">
                                <h5 className="text-[10px] text-gray-500 font-bold uppercase mb-2 border-t border-dark-border pt-2">Banco</h5>
                                <div className="space-y-1">
                                    {awaySquad.bench.map(player => ( <PlayerStatsRow key={player.id} player={player} events={events} currentMatchMinute={currentMatchMinute} /> ))}
                                </div>
                             </div>
                        </div>
                     </div>
