...
import { 
  ...
  Shield, Hand, Square, UserMinus, UserPlus, Footprints, Target, Trash2, PlayCircle
} from 'lucide-react';
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-5/12 flex flex-col border-r border-dark-border bg-black relative">
            ...
            <div className="flex-1 bg-dark-surface p-4 overflow-hidden flex flex-col">
                <h3 className="text-[10px] font-bold uppercase text-gray-500 mb-3 tracking-widest border-b border-dark-border pb-2">Feed de Eventos</h3>
                <div className="flex-1 overflow-y-auto space-y-1">
                    {events.map(ev => (
                        <div key={ev.id} className="flex items-center justify-between gap-2 text-xs p-2 bg-dark-bg rounded border border-dark-border/50 hover:border-gray-600 group">
                            <div className="flex items-center gap-3">
                                <span className="font-mono text-brand font-bold w-6">{ev.match_minute}'</span>
                                <span className="text-gray-300 font-bold w-28 truncate">{ev.player?.name || 'Geral'}</span>
                                <span className="text-gray-500 text-[10px] uppercase">{ev.type.replace('_', ' ')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <button 
                                    onClick={() => playerRef.current?.seekTo(ev.video_timestamp, 'seconds')}
                                    className="text-gray-500 hover:text-brand transition mr-2"
                                    title="Ver no vídeo"
                                >
                                    <PlayCircle size={14} />
                                </button>
                    ...
                    {events.length === 0 && <div className="text-gray-600 text-[10px] text-center italic mt-10">Nenhum registo recente...</div>}
                </div>
            </div>
        </div>
