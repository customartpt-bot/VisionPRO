...
import { Plus, Trash2, Monitor, PlayCircle, Calendar, Trophy, User } from 'lucide-react';

const MatchList: React.FC = () => {
  const [matches, setMatches] = useState<Match[]>([]);
  const navigate = useNavigate();
  // Fetch role directly in render/handlers to ensure freshness
  ...
  const handleMatchClick = (matchId: string) => {
      ...
      if (currentRole === 'dashboard') {
          navigate(`/dashboard/${matchId}`);
      } else if (currentRole === 'analistaind') {
          navigate(`/matches/${matchId}/individual-console`);
      } else {
          // Admin e Analista Coletivo vão para a consola principal
          navigate(`/matches/${matchId}/console`);
      }
  };
        ...
        <div className="flex justify-between items-end mb-12 border-b border-dark-border pb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col items-end">
               ...
               <span className="text-[10px] text-brand font-mono mt-1">{role?.toUpperCase()}</span>
            </div>
          </div>
        </div>
            ...
            >
               ...
               <div className="bg-dark-bg p-3 flex justify-center items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-400 group-hover:text-brand transition-colors border-t border-dark-border">
                  {role === 'dashboard' ? (
                      ...
                      <><User size={14} /> Abrir Consola Individual</>
                  ) : (
                      <><PlayCircle size={14} /> Abrir Consola Principal</>
                  )}
               </div>
            </div>
          ))}
