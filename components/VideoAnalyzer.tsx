...
import { dbService } from '../services/dbService'; // Importar dbService
import { AnalysisData } from '../types'; // Importar AnalysisData

interface VideoAnalyzerProps {
  onAnalyze: (base64: string, mimeType: string) => Promise<AnalysisData>; // onAnalyze agora retorna Promise<AnalysisData>
  isAnalyzing: boolean;
}
...
const VideoAnalyzer: React.FC<VideoAnalyzerProps> = ({ onAnalyze, isAnalyzing }) => {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      const aiAnalysisResult = await onAnalyze(base64, videoFile.type); // Chamar a função de análise da IA
      
      // Salvar os resultados da análise da IA no banco de dados
      await dbService.saveAIAnalysis(aiAnalysisResult);
      alert("Análise de IA concluída e salva com sucesso!");
    } catch (err) {
      alert("Erro ao processar ficheiro de vídeo ou salvar análise.");
      console.error(err);
    }
  };
    ...
    <div className="glass-card rounded-3xl p-8 border border-white/5 shadow-2xl relative">
      ...
      {!previewUrl ? (
        ...
        >
          ...
          <p className="text-zinc-400 font-bold text-[11px] uppercase tracking-widest">Carregar Vídeo para Análise Tática</p>
          <p className="text-[9px] text-zinc-600 uppercase mt-2 font-bold tracking-tighter">O motor analisará os frames reais para extrair estatísticas.</p>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-white/5 aspect-video bg-black">
          ...
          {isAnalyzing && (
            <div className="absolute inset-0 bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center z-50">
              ...
              <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">IA_ANALISANDO_FRAMES_REAIS</h3>
              <div className="mt-4 flex flex-col gap-1 text-[9px] font-bold uppercase mono">
                 <p className="text-emerald-500">>> EXTRACTING_PITCH_DATA</p>
                 <p className="text-orange-500">>> VALIDATING_METRICS</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mt-8 space-y-4">
        ...
        >
          {isAnalyzing ? "EXTRAÇÃO EM CURSO..." : "EXTRAIR ESTATÍSTICAS REAIS"}
        </button>
      </div>
    </div>
  );
