...
import { 
  Mic, MicOff, Play, Pause, ArrowLeft, Layers, User, RotateCcw, 
  Clock, CheckCircle2, XCircle, Crosshair, Shield, Hand, Timer, Activity, Goal,
  AlertTriangle, RectangleHorizontal, ShieldAlert, Ban, ArrowRightLeft, Trash2
} from 'lucide-react';
...
const AnalysisConsole: React.FC = () => {
  ...
  const processVoiceCommand = (text: string) => {
    let foundPlayer = players.find(p => text.includes(p.name.toLowerCase()));
    if (!foundPlayer) return;
    let eventType: EventType | null = null;
    if (text.includes('passe') || text.includes('bom')) eventType = 'pass_success';
    else if (text.includes('errado') || text.includes('mau')) eventType = 'pass_fail';
    else if (text.includes('golo')) eventType = 'goal';
    else if (text.includes('remate')) eventType = 'shot_on_target';
    if (eventType) addEvent(eventType, foundPlayer.team, foundPlayer.id);
  };
      ...
      <div className="bg-black border-b border-dark-border h-16 flex items-center justify-between px-6 shrink-0 z-20 relative">
        <div className="flex items-center gap-4 w-1/3">
          ...
          <div className="flex flex-col ml-2">
             <div className="flex items-center gap-2 font-bold uppercase tracking-wider text-sm leading-none">
                <span className="text-white">{match?.home_team}</span>
                <span className="text-brand text-[10px]">VS</span>
                <span className="text-white">{match?.away_team}</span>
             </div>
             <div className="text-[10px] text-gray-500 font-mono mt-1">{match?.competition}</div>
          </div>
        </div>

        {/* Scoreboard Central - COMPACT MODE (ROW) */}
        ...
        <div className="flex items-center gap-4 w-1/3 justify-end">
           <div className="text-right mr-4 hidden md:block">
             <div className="text-[10px] uppercase text-gray-600 font-bold">Operador</div>
             <div className="text-xs font-bold text-brand">{role?.toUpperCase()}</div>
           </div>
           <button onClick={toggleVoice} className={`p-3 rounded-full transition-all border ${isVoiceActive ? 'text-brand bg-brand/10 border-brand shadow-[0_0_10px_rgba(255,77,0,0.3)]' : 'text-gray-600 border-gray-800 hover:text-white hover:border-gray-600'}`}>
